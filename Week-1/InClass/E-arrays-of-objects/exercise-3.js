...
let restaurantFinderApplication = {
	...
	findAvailableRestaurants: function(numberOfPeople) {
		let availableRestaurantArray = this.restaurants.filter(function(restaurants) {
			return numberOfPeople <= restaurants.totalSeats - restaurants.numberOfCustomers;
		});
		let availableRestaurantName = '';
		for (var i = 0; i < availableRestaurantArray.length; i++) {
			availableRestaurantName += availableRestaurantArray[i].name + ',';
		}
		return availableRestaurantName;
	},
	findRestaurantServingDish: function(dishName) {
		let restaurantServingDishArray = this.restaurants.filter(function(restaurants) {
			return restaurants.menu.includes(dishName);
		});
		let availableRestaurantName = '';
		for (var i = 0; i < restaurantServingDishArray.length; i++) {
			availableRestaurantName += restaurantServingDishArray[i].name + ',';
		}
		return availableRestaurantName;
	},
	countNumberOfRestaurantsInArea: function(area) {
		let restaurtInAreaArray = this.restaurants.filter(function(restaurants) {
			return restaurants.address.area === area;
		});

		return restaurtInAreaArray.length;
	}
};
